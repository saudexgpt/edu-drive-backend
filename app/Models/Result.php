...
class Result extends Model
{
    ...
    public function updateResultDetails($result_detail, $result_settings)
    {
        ...
        $test = $this->addCaScores($result_detail, $result_settings);
        $result_detail->total = $test + $result_detail->exam;
        $result_detail->save();


        $result_detail->test = $test;
    ...
    public function addCaScores($result_detail, $result_settings)
    {
        ...
        ///////////////////WE  HAVE DISABLED THE CONVERSION OF ATTENDANCE SCORES TO CA////////
        ...
        if (!empty($grades)) {
            foreach ($grades as $grade_detail) {
                ...
                if ($total == null) {
                    ...
                    $grade_point = sprintf("%01.1f", $grade_detail->grade_point);
    ...
    public function processStudentResults($student_results, $options)
    {
        ...
        foreach ($student_results as $student_result) :
            ...
            $student_cummulative_result = Result::groupBy('subject_teacher_id')->where([
                'subject_teacher_id' => $subject_teacher_id,
                'school_id' => $options['school_id'],
                'sess_id' => $options['sess_id'],
    ...
    public function analyseStudentsResult($student, $options)
    {
        ...
        ])/*->where('teacher_id', '!=', NULL)*/->orderBy('subjects.name')->select('subject_teachers.id as id', 'subjects.name as subject_name')->get();
        ...
        if ($class_subjects->isNotEmpty()) {
            ...
            foreach ($class_subjects as $subject_teacher) :
                ...
                $subject_name = $subject_teacher->subject_name;
                ...
                )->where($approval_status, '!=', 'Not Submitted')->first();
                if ($student_result) {
